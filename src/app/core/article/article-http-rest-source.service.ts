...
export class ArticleHttpRestSource implements ArticleSource {
  ...
  public getArticle(id: number): Observable<Article> {
    const article = this.getArticles().pipe(map((articles) => articles?.find((article) => article.id == id)));
  ...
  public getArticlesOfAuthor(name: string): Observable<Article[]> {
    const articles = this.getArticles().pipe(map((articles) => articles?.filter((article) => article.author == name)));
    return articles as Observable<Article[]>;
  }

  public getNumberArticlesFromAuthor(name: string): Observable<number> {
    const articles = this.getArticles().pipe(map((articles) => articles?.filter((article) => article.author == name)));
    return articles.pipe(map((articles) => articles.length));
  }
